...
--(88,029 rows affected)

--Join ##invol to get Medicaid 
...
,sum(invol_flag) as count_admission
...
--select * from ##inpatient_mh
--select * from ##invol_w_p1 where Medicaid_flag = 1

/*Part 3.2: MH, Voluntary, Medicaid */
...
/*
--MH, Involuntary, Medicaid
DROP TABLE IF EXISTS ##mh_invol_mcaid

SELECT DISTINCT 
a.kcid
,a.admit_date
,Medicaid_flag as medicaid_flag
,invol_flag
,isnull(b.P1_FLAG,0) as P1_FLAG 
INTO ##mh_invol_mcaid
FROM (select * from ##invol_w_p1 where Medicaid_flag = 1) a
LEFT JOIN (SELECT *, 1 AS P1_FLAG FROM ##inpatient_mh) b
on a.admit_date =b.admit_date
and a.p1_id=b.id_mcaid
--(9340 rows affected)

select P1_FLAG, count (*) from  ##mh_invol_mcaid group by P1_FLAG
*/
...
Select 
DATEPART (YEAR,[admit_date]) AS admit_year
,sum(vol_flag) as count_admission
...
WHERE cc.[call_stamp] IS NOT NULL
 ...
 AND cc.call_stamp between @StartDate and @EndDate